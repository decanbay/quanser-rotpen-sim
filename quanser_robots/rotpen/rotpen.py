import numpy as np
from .base import RotpenBase, RotpenDynamics


class Rotpen(RotpenBase):
    def __init__(self, fs, fs_ctrl):
        super(Rotpen, self).__init__(fs, fs_ctrl)
        self.dyn = RotpenDynamics()
    ...
    def _set_gui(self):
        ...
        self._vis['vp'].scene.forward = self._vis['vp'].vector(-1, 0.5, -0.5)
        self._vis['vp'].box(pos=self._vis['vp'].vector(0, 0, -0.07),
                     length=0.12, width=0.12, height=0.12,
    ...
    def _calibrate(self):
        self._sim_state = \
            0.01 * np.float16(self._np_random.randn(self.state_space.shape[0]))
        self._state = self._zero_sim_step()

    def _sim_step(self, a):
        thdd, aldd = self.dyn(self._sim_state, a)
        ...
        return np.copy(self._sim_state)
