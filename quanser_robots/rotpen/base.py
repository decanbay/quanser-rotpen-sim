...
class RotpenBase(gym.Env):
    def __init__(self, fs, fs_ctrl):
        super(RotpenBase, self).__init__()
        self._state = None
...
class RotpenDynamics:
    ...
    def __init__(self):
        ...
        # Motor
        self.Rm = 2.6    # resistance
        self.km = 0.0077  # back-emf constant (V-s/rad)

        # Rotary arm
        self.Mr = 0.2570  # mass (kg)
        self.Lr = 0.2159  # length (m)
        self.Dr = 0.0024   # viscous damping (N-m-s/rad), original: 0.0015

        # Pendulum link
        self.Mp = 0.1270  # mass (kg)
        self.Lp = 0.3365  # length (m)
        self.Dp = 0.0024   # viscous damping (N-m-s/rad), original: 0.0005
